...
def draw_cube_with_image(image_path):
    ...
    try:
        ...
        print(f"Failed to open the image file: {image_path}")
...
def draw_scene(flatColors=False):
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            for k in range(n):
                ...
                textureId = loadTexture("Trab2/arrow2.jpg") 
    ...
    if len(removed) == n**3:
        draw_cube_with_image("Trab2/youwin.png")  # Replace with the actual image path
...
def mousePressed(button, state, x, y):
    ...
    if state == GLUT_DOWN:
        ...
        print(selected)
        print(right_translation[selected])
